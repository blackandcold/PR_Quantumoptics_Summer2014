...
\\
\textbf{Bell's formulation}\\
John Bell proposed a inequality assuming that every mean of measurement values has a bound value. This inequality is satisfied for systems based on realism and locality. It assumes that the outcome of experiments are "known beforehand" (except a hidden variable $\lambda$ $\rightarrow$ CHSH). This means if one measures a correlated spin of a system of 2 particles along different directions $\alpha$ and $\beta$ the expectation value is deterministic.\\
$$A(\vec{a},\lambda) = \pm 1, B(\vec{b},\lambda) = \pm 1$$
$$E(\vec{a},\vec{b}) = \int d\lambda \rho(\lambda)\cdot A(\vec{a},\lambda) \cdot B(\vec{b},\lambda)$$
with another vector $\vec{c}$:
...
\subsection{Build}
In the following figure, a sketch of the experiment is shown. The goal is, to send a laser beam through a BBO, which will then create single photons. The propagating entangled photons are impinging on the detector (through 2 coupling-arms) where coincidences can be observed.
\begin{figure}[H]
 \centering
...
In this experiment, a laser with wavelength of 405nm is used. Its power can be controlled by software and is variable between 1mW and 50 mW. For aligning purposes, the power is usually set to approximately 2-5mW, for the measurement, the full power of 50mW is used.\\
\\
The laser beam is  focused on the BBO, using 2 mirrors (1 close to the laser to produce transitional motion, the other one near the BBO to control the angle). To make sure the beam hits the BBO as focused as possible, a lens with a focal length of 25 cm is placed in between. Using the available irises helps to increase accuracy.\\ %The BBO is absolutely necessary since there the spontaneaous parametric down-conversion (SPDC) takes place, which is the source of the entangled photons,for further information go to \ref{SPDC}. 
\\
Behind the BBO, 2 cones of entangled photons are emitted, one of them e-polarized (extraordinary) and the other o-polarized (ordinary)(\ref{fig:cones}). Between the pump beam and the interception points of the cones, there is an angle of $3^\circ$. To measure up with coincidences it is  necessary to locate the signal and the interception points respectively. Therefore, the path the 2 beams take, have to be symmetric. Since the beams are not visible, it is hard to align them perfectly.\\
To do this, laser pointers are used. They are plugged into the fibre cables of the detector and sent in the reverse direction. Now, the visible red dots can be aligned, producing the path of the entangled photons in reverse.\\
\\
After passing the two prisms (that act as mirrors), one has practically two separate arms. These two arms should be symmetric. \\
In the beam, a second BBO with half width of the first one and a polarizer are placed. The second BBO is needed to make sure that the photons arrive at the same time. This is necessary because the photons emitted along the ordinary beam are faster. In the end, the beam should impinge on the detector precisely to get as much counts as possible. The detector-input itself is placed on a translation stage, leaving 4 degrees of freedom, 2 spatially and 2 angles.\\
It is important to work in a dark room, while the detector is switched on due to the fact that it is highly sensitive to light and could get damaged by to much light.\\
\\
As mentioned before, the aim is to get the beam at the interception points. To find these, the second BBO is not needed, instead, a polarizer is inserted into the beam. Now, one measures the count as function of the polarization to get $50\%$ horizontal and $50\%$ vertical polarization. To get there, the detector-input is moved through the beam. By observing the count rate, it is possible to scan the cones up and down to get to the favored position. Afterwards, the cones are scanned left and right to find the local maximum. This should now be the interception point.\\
